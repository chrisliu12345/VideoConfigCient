//changed by konglingyuan 

(function () {


    angular
        .module('template/treeGrid/treeGrid.html', ['ngAnimate', 'ui.bootstrap'])
        .run([
            '$templateCache',
            function ($templateCache) {
                $templateCache.put('template/treeGrid/treeGrid.html',
                    ...
                    "         <div ng-if=\"!col.cellTemplate\">{{row.branch[col.field]}}"+
                    // "             <button type=\"button\" class=\"btn btn-primary popup-marker\" title=\"Popover title\" "+ 
                    //                 "data-container=\"body\" data-toggle=\"popover\" data-placement=\"top\" "+
                    //                         "data-content=\"顶部的 Popover 中的一些内容\" ng-click=\"toggle_detail(row.branch)\">menu</button>"+
                                        
                                  "<i ng-if=(row.branch.personType==\"user\") popover-template=\"'popover.html'\" "+
                                    "data-container=\"body\" popover-placement=\"top\" "+
                                    "popover-trigger=\"click\" "+
                                    "type=\"button\""+
                                    "class=\"indented tree-icon glyphicon glyphicon-comment\" ng-click=\"toggle_detail(row.branch,'app')\">"+
                                  "</i>  "+
                                   "<i ng-if=(row.branch.personType==\"user\") popover-template=\"'mailPopover.html'\" "+
                                    "data-container=\"body\" popover-placement=\"top\" "+
                                    "popover-trigger=\"click\" "+
                                    "type=\"button\""+
                                    "class=\"indented tree-icon glyphicon glyphicon-envelope\" ng-click=\"toggle_detail(row.branch,'mail')\">"+
                                  "</i>  "+
                                   "<i ng-if=(row.branch.personType==\"user\") popover-template=\"'phonePopover.html'\" "+
                                    "data-container=\"body\" popover-placement=\"top\" "+
                                    "popover-trigger=\"click\" "+
                                    "type=\"button\""+
                                    "class=\"indented tree-icon glyphicon glyphicon-earphone\" ng-click=\"toggle_detail(row.branch,'phone')\">"+
                                  "</i>  "+
                              "</div>\n" +
                    "       </td>\n" +
                    "     </tr>\n" +
                    ...
                    " </table>\n" +
                        "<script type=\"text/ng-template\" id=\"popover.html\">"+
                         " <div style=\"z-index: 9999;\">"+
                            "<h3 class=\"popover-title\">应用信息</h3>"+
                            "<div class=\"popover-content\">"+
                            "   <div ng-repeat=\"app in appList\">"+
                            "       <a href=\"javascript:void(0)\" ng-click=\"user_clicks_branch(app.obj)\">{{app.name}}</a>"+
                            "   </div>"+
                            "</div>"+
                            // "<div class=\"popover-content\"><a href=\"javascript:void(0)\" ng-click=\"user_clicks_branch({{clickUrl}})\">{{538路}}</a></div> "+
                          "</div>"+
                        "</script>"+
                        "<script type=\"text/ng-template\" id=\"mailPopover.html\">"+
                         " <div style=\"z-index: 9999;\">"+
                            "<h3 class=\"popover-title\">邮箱信息</h3>"+
                            "<div class=\"popover-content\">"+
                            "   <div ng-repeat=\"mail in mailList\">"+
                            "       <a href=\"javascript:void(0)\" ng-click=\"user_clicks_branch(mail.obj)\">{{mail.name}}</a>"+
                            "   </div>"+
                            "</div>"+
                            // "<div class=\"popover-content\"><a href=\"javascript:void(0)\" ng-click=\"user_clicks_branch({{clickUrl}})\">{{538路}}</a></div> "+
                          "</div>"+
                        "</script>"+
                        "<script type=\"text/ng-template\" id=\"phonePopover.html\">"+
                         " <div style=\"z-index: 9999;\">"+
                            "<h3 class=\"popover-title\">电话信息</h3>"+
                            "<div class=\"popover-content\">"+
                            "   <div ng-repeat=\"phone in phoneList\">"+
                            "       <a href=\"javascript:void(0)\" ng-click=\"user_clicks_branch(phone.obj)\">{{phone.name}}</a>"+
                            "   </div>"+
                            "</div>"+
                            // "<div class=\"popover-content\"><a href=\"javascript:void(0)\" ng-click=\"user_clicks_branch({{clickUrl}})\">{{538路}}</a></div> "+
                          "</div>"+
            ...
            function ($timeout,
                ...
                return {
                    ...
                    link: function (scope, element, attrs) {
                        ...
                        //点击每一行的菜单，弹出相应的页面
                        scope.toggle_detail = function(branch,type){
                            // console.log(type);
                            if(type=="app"){
                                scope.appList=[];
                                branch.children.forEach(function(d,i){
                                    var tempObj={};
                                    tempObj.obj=d;
                                    tempObj.name=d.NAME;
                                    scope.appList.push(tempObj);
                                });
                            }else if(type=="mail"){
                                scope.mailList=[];
                                var tempObj={};
                                tempObj.obj=branch;
                                tempObj.name=branch.mail;
                                scope.mailList.push(tempObj);
                            }else if(type=="phone"){
                                scope.phoneList=[];
                                var tempObj={};
                                tempObj.obj=branch;
                                tempObj.name=branch.phone;
                                scope.phoneList.push(tempObj);
                            }
